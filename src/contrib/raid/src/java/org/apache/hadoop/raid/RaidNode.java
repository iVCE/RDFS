...
public abstract class RaidNode implements RaidProtocol {
	...
	public static enum LOGTYPES {
		ONLINE_RECONSTRUCTION, OFFLINE_RECONSTRUCTION, ENCODING
	};

	public static final Log LOG = LogFactory.getLog(RaidNode.class);
	public static final Log ENCODER_METRICS_LOG = LogFactory
			.getLog("RaidMetrics");
	public static final long SLEEP_TIME = 10000L; // 10 seconds
	public static final String TRIGGER_MONITOR_SLEEP_TIME_KEY = "hdfs.raid.trigger.monitor.sleep.time";
	public static final int DEFAULT_PORT = 60000;
	// we don't raid too small files
	public static final long MINIMUM_RAIDABLE_FILESIZE = 10 * 1024L;
	public static final String MINIMUM_RAIDABLE_FILESIZE_KEY = "hdfs.raid.min.filesize";

	public static final String RAID_RECOVERY_LOCATION_KEY = "hdfs.raid.local.recovery.location";
	public static final String DEFAULT_RECOVERY_LOCATION = "/tmp/raidrecovery";

	public static final String RAID_PARITY_HAR_THRESHOLD_DAYS_KEY = "raid.parity.har.threshold.days";
	...
	public static final String RAID_DISABLE_CORRUPT_BLOCK_FIXER_KEY = "raid.blockreconstruction.corrupt.disable";
	public static final String RAID_DISABLE_DECOMMISSIONING_BLOCK_COPIER_KEY = "raid.blockreconstruction.decommissioning.disable";
	public static final String RAID_DISABLE_CORRUPTFILE_COUNTER_KEY = "raid.corruptfile.counter.disable";
	...
	public static class Statistics {
		...
		public String toString() {
			...
			String msg = " numProcessedBlocks = " + numProcessedBlocks
					+ " processedSize = " + processedSize + " postRaidSize = "
					+ remainingSize + " numMetaBlocks = " + numMetaBlocks
					+ " metaSize = " + metaSize + " %save in raw disk space = "
					+ savep;
			return msg;
		}
	}
	 ...
	 * RaidNode is up and running if the user passes the port as
	...
	public void join() {
		try {
			...
			if (triggerThread != null)
				triggerThread.join();
			if (blockFixerThread != null)
				blockFixerThread.join();
			if (blockCopierThread != null)
				blockCopierThread.join();
			if (corruptFileCounterThread != null)
				corruptFileCounterThread.join();
			if (purgeThread != null)
				purgeThread.join();
			if (statsCollectorThread != null)
	...
	public void stop() {
		...
		if (blockIntegrityMonitor != null)
			blockIntegrityMonitor.running = false;
		if (blockFixerThread != null)
			blockFixerThread.interrupt();
		if (blockCopierThread != null)
			blockCopierThread.interrupt();
		if (corruptFileCounterThread != null)
			corruptFileCounterThread.interrupt();
		if (purgeMonitor != null)
			purgeMonitor.running = false;
		if (purgeThread != null)
		...
		if (statsCollector != null)
			statsCollector.stop();
		if (statsCollectorThread != null)
			statsCollectorThread.interrupt();
		if (infoServer != null) {
			try {
				infoServer.stop();
			} catch (Exception e) {
	...
	private void startHttpServer() throws IOException {
		String infoAddr = conf.get("mapred.raid.http.address",
				"localhost:50091");
		InetSocketAddress infoSocAddr = NetUtils.createSocketAddr(infoAddr);
		this.infoBindAddress = infoSocAddr.getHostName();
		int tmpInfoPort = infoSocAddr.getPort();
		this.infoServer = new HttpServer("raid", this.infoBindAddress,
				tmpInfoPort, tmpInfoPort == 0, conf);
		this.infoServer.setAttribute("raidnode", this);
		this.infoServer.addInternalServlet("corruptfilecounter",
				"/corruptfilecounter", CorruptFileCounterServlet.class);
	...
	public BlockIntegrityMonitor.Status getBlockFixerStatus() {
		return ((Worker) blockIntegrityMonitor.getCorruptionMonitor())
				.getStatus();
	}

	public BlockIntegrityMonitor.Status getBlockCopierStatus() {
		return ((Worker) blockIntegrityMonitor.getDecommissioningMonitor())
				.getStatus();
	}
	...
	/** {@inheritDoc} */
	public String recoverFile(String inStr, long corruptOffset)
			throws IOException {
	...
	class TriggerMonitor implements Runnable {
		...
		private boolean shouldSelectFiles(PolicyInfo info) {
			...
			if (scanState.isScanInProgress()) {
				...
				// Check the time of the last full traversal before starting a
				// fresh
				// traversal.
		...
		private List<FileStatus> readFileList(PolicyInfo info)
				throws IOException {
			...
			// Max number of files returned.
			int selectLimit = configMgr.getMaxFilesPerJob();
			int targetReplication = Integer.parseInt(info
					.getProperty("targetReplication"));
			...
			if (!scanState.isFileListReadInProgress()) {
				scanState.startTime = now();
				try {
					InputStream in = fileListPath.getFileSystem(conf).open(
							fileListPath);
					scanState.fileListReader = new BufferedReader(
			...
			try {
				while ((l = scanState.fileListReader.readLine()) != null) {
					...
					if (codec.isDirRaid) {
						...
						List<FileStatus> lfs = RaidNode
								.listDirectoryRaidFileStatus(conf, fs, p);
						if (lfs == null) {
							continue;
						}
		...
		/**
		 * Keep processing policies. If the config file has changed, then reload
		 * config file and start afresh.
		 */
			...
			while (running) {
				...
				for (PolicyInfo info : allPolicies) {
					...
					if (shouldReadFileList(info)) {
						...
						try {
							filteredPaths = selectFiles(info,
									allPoliciesWithSrcPath);
						} catch (Exception e) {
							LOG.info("Exception while invoking filter on policy "
									+ info.getName()
									+ " srcPath "
									+ info.getSrcPath()
									+ " exception "
					...
					if (filteredPaths == null || filteredPaths.size() == 0) {
						LOG.info("No filtered paths for policy "
								+ info.getName());
						continue;
					}
					...
					try {
						...
						LOG.info(
								"Exception while invoking action on policy "
										+ info.getName() + " srcPath "
										+ info.getSrcPath() + " exception "
	...
	void doRaid(Configuration conf, PolicyInfo info, List<FileStatus> paths)
			throws IOException {
		int targetRepl = Integer
				.parseInt(info.getProperty("targetReplication"));
		...
		for (FileStatus s : paths) {
			doRaid(conf, s, destPref, codec, statistics,
					RaidUtils.NULL_PROGRESSABLE, doSimulate, targetRepl,
					metaRepl);
			if (count % 1000 == 0) {
	...
	static public boolean doRaid(Configuration conf, PolicyInfo info,
		...
		return doRaid(conf, src, destPref, codec, statistics, reporter,
	...
	public static boolean doRaid(Configuration conf, FileStatus stat,
			...
			Progressable reporter, boolean doSimulate, int targetRepl,
			int metaRepl) throws IOException {
		long startTime = System.currentTimeMillis();
		boolean success = false;
		try {
			if (codec.isDirRaid) {
				success = doDirRaid(conf, stat, destPath, codec, statistics,
						reporter, doSimulate, targetRepl, metaRepl);
			} else {
				success = doFileRaid(conf, stat, destPath, codec, statistics,
						reporter, doSimulate, targetRepl, metaRepl);
			}
			return success;
			...
			if (success) {
				logRaidEncodingMetrics("SUCCESS", codec, delay,
						statistics.processedSize,
						statistics.numProcessedBlocks,
	...
	public static boolean doDirRaid(Configuration conf, FileStatus stat,
		...
		generateParityFile(conf, stat, targetRepl, reporter, srcFs, destPath,
				codec, blockNum, srcRepl, metaRepl, parityBlockSize, lfs);
		if (!doSimulate) {
			for (FileStatus fsStat : lfs) {
		...
		// the last block of the metafile might not be completely filled up, but
		// we
		// ignore that for now.
	...
	public static boolean doFileRaid(Configuration conf, FileStatus stat,
		...
		BlockLocation[] locations = srcFs.getFileBlockLocations(stat, 0,
				stat.getLen());
		...
		// generate parity file
		generateParityFile(conf, stat, targetRepl, reporter, srcFs, destPath,
				codec, locations.length, stat.getReplication(), metaRepl,
				stat.getBlockSize(), null);
		if (!doSimulate) {
			if (srcFs.setReplication(p, (short) targetRepl) == false) {
				LOG.info("Error in reducing replication of " + p + " to "
						+ targetRepl);
				statistics.remainingSize += diskSpace;
				return false;
			}
		...
		// we create numMeta for every file. This metablock has metaRepl #
		// replicas.
		// the last block of the metafile might not be completely filled up, but
		// we
		// ignore that for now.
	...
	static private void generateParityFile(Configuration conf, FileStatus stat,
			int targetRepl, Progressable reporter, FileSystem inFs,
			Path destPathPrefix, Codec codec, long blockNum, int srcRepl,
			int metaRepl, long blockSize, List<FileStatus> lfs)
			throws IOException {
		...
		try {
			...
			if (stmp.getModificationTime() == stat.getModificationTime()
					&& srcRepl == targetRepl) {
				LOG.info("Parity file for " + inpath + "(" + blockNum + ") is "
						+ outpath + " already upto-date and "
		...
		if (codec.isDirRaid) {
			long numStripes = (blockNum % codec.stripeLength == 0) ? (blockNum / codec.stripeLength)
					: ((blockNum / codec.stripeLength) + 1);

			sReader = new DirectoryStripeReader(conf, codec, inFs, 0, inpath,
					lfs);
			...
			long numBlocks = (srcSize % blockSize == 0) ? (srcSize / blockSize)
					: ((srcSize / blockSize) + 1);
			long numStripes = (numBlocks % codec.stripeLength == 0) ? (numBlocks / codec.stripeLength)
					: ((numBlocks / codec.stripeLength) + 1);
			sReader = new FileStripeReader(conf, blockSize, codec, inFs, 0,
					inpath, srcSize);
			encoder.encodeFile(conf, inFs, inpath, outFs, outpath,
					(short) metaRepl, numStripes, blockSize, reporter, sReader);
		}

		// set the modification time of the RAID file. This is done so that the
		// modTime of the
		// RAID file reflects that contents of the source file that it has
		// RAIDed. This should
		// also work for files that are being appended to. This is necessary
		// because the time on
		// on the destination namenode may not be synchronised with the
		// timestamp of the
		// source namenode.
		...
		if (outstat.getModificationTime() != inStat.getModificationTime()) {
			String msg = "Parity file mtime " + outstat.getModificationTime()
					+ " does not match source mtime "
					+ inStat.getModificationTime();
			throw new IOException(msg);
		}
		LOG.info("Source file " + inpath + " of size " + inStat.getLen()
				+ " Parity file " + outpath + " of size " + outstat.getLen()
				+ " src mtime " + stat.getModificationTime() + " parity mtime "
				+ outstat.getModificationTime());
	}

	public static DecoderInputStream unRaidCorruptInputStream(
			Configuration conf, Path srcPath, Codec codec,
			ParityFilePair parityFilePair, long blockSize, long corruptOffset,
			long limit) throws IOException {
		// Test if parity file exists
		if (parityFilePair == null) {
			LOG.warn("Could not find " + codec.id + " parity file for "
					+ srcPath);
			return null;
		}
		...
		while (running) {
			...
			for (Codec codec : Codec.getCodecs()) {
				...
				try {
					...
					int harThresold = conf.getInt(
							RAID_PARITY_HAR_THRESHOLD_DAYS_KEY,
					...
					recurseHar(codec, destFs, destStat, destPref.toUri()
							.getPath(), destFs, cutoff, tmpHarPath);
				} catch (Exception e) {
	...
	public static RaidNode createRaidNode(Configuration conf)
		...
		try {
			...
			Class<?> raidNodeClass = conf.getClass(RAIDNODE_CLASSNAME_KEY,
					DistRaidNode.class);
			if (!RaidNode.class.isAssignableFrom(raidNodeClass)) {
				throw new ClassNotFoundException(
						"not an implementation of RaidNode");
			}
			Constructor<?> constructor = raidNodeClass
