...
public class SimpleRegeneratingCode extends ErasureCode {
	public static final Log LOG = LogFactory
			.getLog(SimpleRegeneratingCode.class);
	...
	public void init(Codec codec) {
		...
		LOG.info(" Initialized " + SimpleRegeneratingCode.class
				+ " stripeLength:" + codec.stripeLength + " parityLength:"
				+ codec.parityLength + " SRC parities:" + paritySizeSRC);
	}
	...
	private void init(int stripeSize, int paritySize) {
		...
		simpleParityDegree = (int) Math
				.ceil((double) (stripeSize + paritySizeRS)
						/ (double) (paritySizeSRC + 1));
	...
	public void encode(int[] message, int[] parity) {
		...
		for (int i = 0; i < paritySizeSRC; i++) {
			parity[i] = 0;
			for (int j = simpleParityDegree * i; j < simpleParityDegree
					* (i + 1); j++)
	...
	public void encode(int[] message, int[] parity, int[] dataBuffer) {
		...
		// put message in the data buffer
		for (int i = 0; i < stripeSize; i++) {
			dataBuffer[i + paritySizeRS] = message[i];
		}

		// calculate RS parities and copy into parity[]
		GF.remainder(dataBuffer, generatingPolynomial);
		for (int i = 0; i < paritySizeRS; i++) {
			parity[i + paritySizeSRC] = dataBuffer[i];
		}

		// restore message in dataBuffer
		for (int i = 0; i < stripeSize; i++) {
			dataBuffer[i + paritySizeRS] = message[i];
		}
		...
		for (int i = 0; i < paritySizeSRC; i++) {
			...
			for (int j = simpleParityDegree * i; j < simpleParityDegree
					* (i + 1); j++)
				parity[i] = GF.add(dataBuffer[j], parity[i]);
		}
	...
	/**
	 * Perform Reed Solomon decoding in a parallel way.
	 * added by jason
	 */
	private void decodeReedSolomonParallel(int[] data, int[] erasedLocations,
			int[] erasedValues) {
		...
		int[] errSignatureLocal = new int[paritySize];
		
		for (int i = 0; i < erasedLocations.length; i++) {
			errSignatureLocal[i] = primitivePower[erasedLocations[i]];
			erasedValues[i] = GF.substitute(data, primitivePower[i]);
		}

		GF.solveVandermondeSystem(errSignatureLocal, erasedValues,
				erasedLocations.length);
	}
	...
	/*
	 * Performs Reed Solomon decoding, assuming that all positions not included
	 * in erasedLocations are available.
	 */
	@Override
	...
	public void decode(int[] data, int[] erasedLocations, int[] erasedValues,
		...
		if (erasedLocations.length == 1) {
			...
			for (int i = 0; i < locationsToRead.length; i++) {
				erasedValues[0] = GF.add(data[locationsToRead[i]],
						erasedValues[0]);
			}
			return;
		}
		...
		assert (locationsNotToRead.length == stripeSize + paritySize
				- locationsToRead.length);
		 ...
		 */
		int[] erasedLocationsRS = new int[locationsNotToRead.length
				- this.paritySizeSRC];
		int k = 0;
		...
		for (int i = 0; i < erasedLocations.length; i++) {
			if (erasedLocations[i] < paritySizeSRC) {
				...
				for (int j = 0; j < groupsTable[erasedLocations[i]].length; j++) {
					data[par] = GF
							.add(data[groupsTable[erasedLocations[i]][j]],
									data[par]);
				}
			}
		}
	 ...
	 * added by jason
	 */
	public void decodeParallel(int[] data, int[] erasedLocations, int[] erasedValues,
			int[] locationsToRead, int[] locationsNotToRead) {
		...
		if (!groupConflict(erasedLocations)) {
			for (int i = 0; i < erasedLocations.length; i++) {
				...
				decodeParallel(data, singleErasedLocation, singleErasedValue,
						groupsTable[erasedLocations[i]], null);
		...
		decodeReedSolomonParallel(dataRS, erasedLocationsRS, erasedValuesRS);
	 ...
	 * The locations are specified as integers in the range [ 0, stripeSize() +
	...
	public List<Integer> locationsToReadForDecode(List<Integer> erasedLocations)
		...
		if (erasedLocations.size() == 1) {
			// locationsToRead =
			// computeSRCGroupLocations(erasedLocations.get(0));
		...
		// If more than one location are erased, check if they belong to the
		// same group.
		// If they do not belong to same group (- no conflict), add locations of
		// each
		// separate group.
		...
		if (!groupConflict(erasedLocationsArray)) {
			// we expect approximately simpleParityDegree locations to be read
			// for each
			// erased location.
			locationsToRead = new ArrayList<Integer>(erasedLocations.size()
					* simpleParityDegree);
		...
		// If more than one location is erased and there is at least a pair
		// belonging to the
		// same group, we will have to perform Reed Solomon (RS) decoding.
		// Hence, read
		...
		// Loop through all possible locations in the stripe, omitting the SRC
		// parities.
	...
	/*
	 * Given a location loc, return a list with the other locations belonging to
	 * the same SRC group as loc.
	 */
	private List<Integer> getSRCGroupNeighbors(int loc) {
		int limit = stripeSize() + paritySize();

		/*
		 * A group is expected to have at most simpleParityDegree + 1 locations.
		 * groupLocations will contain the simpleParityDegree neighbors of loc.
		 */
		...
		if (group < paritySizeSRC) {
			...
			for (int i = paritySizeSRC + group * simpleParityDegree; i < paritySizeSRC
					+ (group + 1) * simpleParityDegree; i++) {
				if (i != loc)
					neighbors.add(i);
			}
	...
	/*
	 * Return the id of the SRC group location loc belongs to. Return -1 for
	 * invalid loc.
	 */
	...
	/*
	 * Check for conflict (- whether any two locations in locs belong to the
	 * same SRC group.
	 */
	private boolean groupConflict(int[] locs) {
		...
		/*
		 * if at least one position in locs is SRC parity, mark the last group.
		 */
	...
	@Override
	public CandidateLocations getCandidateLocations(Stripe stripe,
	...
	@Override
	int[] getRecoverVector(int[] dataLocations, int locationToReconstruct) {
		// TODO Auto-generated method stub
		return null;
	}
