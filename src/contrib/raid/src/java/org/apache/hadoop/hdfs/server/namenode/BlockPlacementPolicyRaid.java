...
public class BlockPlacementPolicyRaid extends BlockPlacementPolicyDefault {
		...
		try {
			...
			if (chosenNodes.size() == 0) {
				...
				if (parity != null) {
					...
					parityBlocks = parityBlocksList
							.toArray(new LocatedBlock[0]);
				}

				final int parityLength = info.codec.parityLength;
				final int stripeLength = info.codec.stripeLength;

				int stripeIndex;
				if (info.type == FileType.TO_RAID) {
					int blockIndex = srcBlocks.length;
					stripeIndex = blockIndex / stripeLength;
				} else if (info.type == FileType.SOURCE) {
					int blockIndex = srcBlocks.length;
					stripeIndex = blockIndex / stripeLength;
				} else {
					int blockIndex = parityBlocks.length;
					stripeIndex = blockIndex / parityLength;
				} 
				
				int startIndex = stripeIndex * stripeLength;
				int endIndex = Math.min(startIndex + stripeLength, srcBlocks.length);
				for (int i = startIndex; i < endIndex; i++) {
					excludedNodes.put(srcBlocks[i].getLocations()[0],
							srcBlocks[i].getLocations()[0]);
				}
				
				if(parityBlocks != null) {
					startIndex = stripeIndex * parityLength;
					endIndex = Math.min(startIndex + parityLength, parityBlocks.length);
					for (int i = startIndex; i < endIndex; i++) {
						excludedNodes.put(parityBlocks[i].getLocations()[0],
								parityBlocks[i].getLocations()[0]);
					}
				}
			...
			chooseRandom(numOfReplicas, "/", excludedNodes, blocksize, 1,
					results);
			//this.cachedLocatedBlocks.addBlock(srcPath, 
			//		new LocatedBlock(new Block(), new DatanodeDescriptor[results.size()]));
  ...
  class NodeComparator implements Comparator<DatanodeDescriptor> {
    ...
    NodeComparator(Map<String, Integer> nodeBlockCount,
  ...
  List<LocatedBlock> getCompanionBlocks(String path, FileInfo info, Block block)
	...
	private String getSourceFile(String path, FileInfo info) {
		Codec codec = info.codec;
		try {
			switch (info.type) {
			case TEMP_PARITY:
				return getSourceFile(path, codec.tmpParityDirectory);
			case PARITY:
				return getSourceFile(path, codec.parityDirectory);
			}
		} catch (IOException e) {
			return null;
		}
  ...
  static class CachedLocatedBlocks extends Cache<String, List<LocatedBlock>> {
    ...
    public void addBlock(String file, LocatedBlock lb) {
    	try {
			List<LocatedBlock> list = this.get(file);
			list.add(lb);
			this.update(file, list);
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
  ...
  private static abstract class Cache<K, V> {
    ...
    void update(K key, V value) {
    	ValueWithTime vt = cache.get(key);
    	vt.value = value;
    	cache.put(key, vt);
    }
	...
	protected FileInfo getFileInfo(String path) throws IOException {
		...
		for (Codec c : Codec.getCodecs()) {
			...
			if (path.startsWith(c.tmpParityDirectory + Path.SEPARATOR)) {
				String tmpStr = path.substring(c.tmpParityDirectory.length() + 1);
				String str = tmpStr.substring(tmpStr.indexOf("/"));
				if(getSourceFile(str, c.parityDirectory) != null)
			...
			if (path.startsWith(c.parityDirectory + Path.SEPARATOR)) {
				if(getSourceFile(path, c.parityDirectory) != null)
		...
		for (org.apache.hadoop.raid.protocol.PolicyInfo policy : policyConfig
				.getAllPolicies()) {
			String srcPath = policy.getSrcPath().toString();
			int index = srcPath.lastIndexOf(':');
			String src = srcPath.substring(index + 1);
			index = src.indexOf('/');
			src = src.substring(index);
			if (path.startsWith(src))
				return new FileInfo(FileType.TO_RAID, Codec.getCodec(policy
						.getCodecId()));
		}
