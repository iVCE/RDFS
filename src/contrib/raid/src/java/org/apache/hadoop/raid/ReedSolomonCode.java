...
public class ReedSolomonCode extends ErasureCode {
  ...
  public void encode(int[] message, int[] parity, int[] dataBuffer) {
    ...
    for (int i = 0; i < stripeSize; i++) {
    	dataBuffer[i + paritySize] = message[i];
    }
    GF.remainder(dataBuffer, generatingPolynomial);
    for (int i = 0; i < paritySize; i++) {
      parity[i] = dataBuffer[i];
    }
  }
  /**
   ...
   * added by jason
   */
  public void decodeParallelLocal(int[] data, int[] erasedLocations, int[] erasedValues) {
	    if (erasedLocations.length == 0) {
	      return;
	    }
	    assert(erasedLocations.length == erasedValues.length);
  ...
  public void decodeParallel(int[] data, int[] erasedLocations, int[] erasedValues,
	     ...
	     */
	    int[] recovValue = new int[locationsNotToRead.length];
	    decodeParallelLocal(data, locationsNotToRead, recovValue);
